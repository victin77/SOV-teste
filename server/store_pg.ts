...
import { Pool } from 'pg';
import { assertStrongPassword } from './password_policy';
...
async function ensureUsersSeededPg(pool) {
  ...
  try {
    ...
    if (bootstrapPass) {
      if (isProd) assertStrongPassword(bootstrapPass, 'bootstrap password', 4);
    ...
    async updateUser(user, updates) {
      ...
      try {
        ...
        const { rows } = await client.query(
          `SELECT username AS user, role, pass_hash AS "passHash", created_at AS "createdAt"
           FROM users
           WHERE username = $1`,
          [user]
        );
        ...
        const passHash = nextPass ? bcrypt.hashSync(String(nextPass), 10) : current.passHash;
        const { rows: updatedRows } = await client.query(
          `UPDATE users
           SET role = $2, pass_hash = $3
           WHERE username = $1
           RETURNING username AS user, role, created_at AS "createdAt"`,
          [user, nextRole, passHash]
        );
        await client.query('COMMIT');
