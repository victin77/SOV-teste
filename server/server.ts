...
import { isStrongPassword } from './password_policy';
...
function isSharedSiteLeadRecord(lead) {
  if (!lead || authSafeStr(lead.stage, 40) !== SITE_STAGE) return false;
...
function canViewLead(auth, lead) {
  if (!auth || !lead || lead.deleted) return false;
  if (auth.role === 'admin' || auth.role === 'leitura') return true;
...
function canEditLead(auth, lead) {
  if (!auth || auth.role === 'leitura') return false;
...
function applyLeadOwnershipRules(auth, existingLead, nextLead) {
  if (!auth || !nextLead) return nextLead;
  if (auth.role === 'admin') return nextLead;

  const me = normalizeUserKey(auth.user);
  if (!me) return { ...nextLead, owner: '' };
...
app.put('/api/admin/users/:user', authMiddleware, requireAdmin, wrapAsync(async (req, res) => {
  ...
  if (!target || target.length > 60) return res.status(400).json({ error: 'invalid_user' });
  if (!role && !pass) return res.status(400).json({ error: 'missing_updates' });
  if (pass && !isStrongPassword(pass, 4)) {
    return res.status(400).json({
      error: 'weak_password',
      details: 'Password must have at least 4 chars.'
    });
  }
  if (roleRaw && !role) return res.status(400).json({ error: 'invalid_role' });
...
function checkRateLimitBucket(bucketMap, key, max, windowMs) {
  ...
  if (bucketMap.size > 5000) {
    for (const [entryKey, bucket] of bucketMap.entries()) {
      if (!bucket || bucket.resetAt <= now) bucketMap.delete(entryKey);
    }
  }
...
function checkSiteFormRateLimit(req) {
  return checkRateLimitBucket(
    siteFormRateBuckets,
    getClientIp(req),
    SITE_FORM_RATE_LIMIT_MAX,
    SITE_FORM_RATE_LIMIT_WINDOW_MS
  );
}
...
app.post('/api/leads', authMiddleware, requireWriter, wrapAsync(async (req, res) => {
...
app.put('/api/leads/:id', authMiddleware, requireWriter, wrapAsync(async (req, res) => {
  const id = req.params.id;
  const existing = await store.getLead(id);
  if (!existing || existing.deleted) return res.status(404).json({ error: 'not_found' });
  if (!canEditLead(req.auth, existing)) return res.status(403).json({ error: 'forbidden' });
...
app.delete('/api/leads/:id', authMiddleware, requireWriter, wrapAsync(async (req, res) => {
  const id = req.params.id;
  const existing = await store.getLead(id);
  if (!existing) return res.status(404).json({ error: 'not_found' });
  if (!canDeleteLead(req.auth, existing)) return res.status(403).json({ error: 'forbidden' });
...
app.post('/api/leads/replace', authMiddleware, requireAdmin, wrapAsync(async (req, res) => {
...
app.get('/api/audit', authMiddleware, requireAdmin, wrapAsync(async (req, res) => {
