...
function setUsersAdminUiState() {
    ...
    if (usersCreateBtn) usersCreateBtn.disabled = !backendOnline || !isAdmin();
    if (usersRefreshBtn) usersRefreshBtn.disabled = !backendOnline || !isAdmin();
    if (usersName) usersName.disabled = !backendOnline || !isAdmin();
    if (usersPass) usersPass.disabled = !backendOnline || !isAdmin();
    if (usersRole) usersRole.disabled = !backendOnline || !isAdmin();

    if (usersHint) {
        if (!backendOnline) usersHint.textContent = 'Servidor offline.';
        else if (!isAdmin()) usersHint.textContent = 'Somente administradores podem gerenciar usuários.';
        else usersHint.textContent = 'Crie acessos para a equipe comercial.';
    }
}
...
function mapImportedStage(value) {
    if (typeof value === 'number' && Number.isFinite(value)) {
        ...
        if (idx >= 0 && idx < STAGES.length) return STAGES[idx];
        if (idx >= 1 && idx <= STAGES.length) return STAGES[idx - 1];
    }
    ...
    if (s.includes('negoci') || s.includes('proposta') || s.includes('deal')) return 'Negociação';
    if (s.includes('simula') || s.includes('cotac') || s.includes('orcament') || s.includes('quote')) return 'Simulação';
    if (s.includes('qualifica') || s.includes('contato') || s.includes('atend') || s.includes('discovery')) return 'Qualificação';
    if (s.includes('site') || s.includes('form') || s.includes('inbound') || s.includes('landing')) return 'Leads do site';
    if (s.includes('novo') || s.includes('new') || s.includes('inicial')) return 'Novo lead';
    return '';
}
...
function buildAppendOnlyImport(existingLeads, importedLeads) {
    ...
    for (const rawLead of incoming) {
        if (!rawLead || typeof rawLead !== 'object') continue;
...
function setImportBusyUI(busy) {
    ...
    if (fileInput) fileInput.disabled = !!busy || !writable;
    if (xlsxFileInput) xlsxFileInput.disabled = !!busy || !writable;
    if (btnImport) btnImport.disabled = !!busy || !writable;
    if (btnImportXlsx) btnImportXlsx.disabled = !!busy || !writable;
...
function handleDateFilter() {
    ...
    if (filterDateFrom) localStorage.setItem('sov_filter_date_from', filterDateFrom);
    else localStorage.removeItem('sov_filter_date_from');

    if (filterDateTo) localStorage.setItem('sov_filter_date_to', filterDateTo);
    else localStorage.removeItem('sov_filter_date_to');
...
function updateViewChrome() {
    ...
    if (filters) filters.style.display = isUsersView ? 'none' : 'flex';
    if (btnNew) btnNew.style.display = isUsersView ? 'none' : '';
}
...
function leadMatchesFilter(lead) {
    ...
    if (hasDateFilter && !leadMs) return false;

    if (fromMs && toMs) {
        const minMs = Math.min(fromMs, toMs);
        const maxMs = Math.max(fromMs, toMs);
        if (leadMs < minMs || leadMs > maxMs) return false;
    } else {
        if (fromMs && leadMs < fromMs) return false;
        if (toMs && leadMs > toMs) return false;
    }
...
function isSharedSiteLead(lead) {
    ...
    if (stage !== 'Leads do site') return false;
...
function getDashboardLeads() {
    ...
    return base.filter((l) => {
        ...
        if (dashboardOwnerFilter === '__none__') return !effectiveOwner;
...
function buildDashboardMarkupV2({
    ownerOptions,
    totalLeads,
    totalValue,
    totalWon,
...
async function saveUserEdit(user) {
    if (!backendOnline) return alert('Servidor offline.');
    if (!isAdmin()) return alert('Apenas admin pode editar usuários.');
    ...
    if (!role && !pass) return alert('Informe um novo perfil ou uma nova senha.');
    if (pass && pass.length < 4) return alert('A nova senha deve ter pelo menos 4 caracteres.');
    ...
    try {
        ...
        if (e && e.code === 'last_admin') return alert('Não é possível remover o último admin da função.');
        if (e && e.code === 'weak_password') return alert('A nova senha precisa ter pelo menos 4 caracteres.');
        if (e && e.code === 'user_not_found') return alert('Usuário não encontrado.');
        if (handleApiFailure(e, 'Não foi possível atualizar o usuário.')) return;
    }
}
...
async function createUserFromUsersView() {
    ...
    try {
        ...
        if (e && e.code === 'user_exists') return alert('Esse usuário já existe.');
        if (e && e.code === 'weak_password') return alert('A senha precisa ter pelo menos 6 caracteres.');
        if (handleApiFailure(e, 'Não foi possível criar o usuário.')) return;
    }
}
