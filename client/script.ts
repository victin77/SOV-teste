...
function mapImportedStage(value) {
    ...
    if (s.includes('negoci') || s.includes('proposta') || s.includes('deal')) return 'NegociaÃ§Ã£o';
    if (s.includes('simula') || s.includes('cotac') || s.includes('orcament') || s.includes('quote')) return 'SimulaÃ§Ã£o';
    if (s.includes('qualifica') || s.includes('contato') || s.includes('atend') || s.includes('discovery')) return 'QualificaÃ§Ã£o';
...
async function saveUserEdit(user) {
    if (!backendOnline) return alert('Servidor offline.');
    if (!isAdmin()) return alert('Apenas admin pode editar usuÃ¡rios.');
    ...
    try {
        ...
        if (e && e.code === 'last_admin') return alert('NÃ£o Ã© possÃ­vel remover o Ãºltimo admin da funÃ§Ã£o.');
        if (e && e.code === 'weak_password') return alert('A nova senha precisa ter pelo menos 4 caracteres.');
        if (e && e.code === 'user_not_found') return alert('UsuÃ¡rio nÃ£o encontrado.');
        if (handleApiFailure(e, 'NÃ£o foi possÃ­vel atualizar o usuÃ¡rio.')) return;
    }
}
