...
async function claimLeadAction(id: string) {
    ...
    if (backendOnline) {
        try {
            ...
            if (handleApiFailure(err, 'Nao foi possivel assumir o lead no servidor.')) return;
            const idx = findLeadIndexById(id);
...
async function saveEditAction(id: string) {
    ...
    if (backendOnline) {
        try {
            ...
            if (handleApiFailure(e as ApiError, 'Nao foi possivel salvar no servidor. Salvando so localmente.')) return;
        }
    }
