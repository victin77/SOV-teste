// @ts-nocheck
const STAGES = ['Leads do site', 'Novo lead', 'QualificaÃ§Ã£o', 'SimulaÃ§Ã£o', 'NegociaÃ§Ã£o', 'Fechado', 'Perdido'];
...
const KANBAN_STAGE_TABS = ['Leads do site', 'Novo lead', 'QualificaÃ§Ã£o', 'SimulaÃ§Ã£o', 'NegociaÃ§Ã£o', 'Fechado', 'Perdido'].filter((s) => STAGES.includes(s));
...
async function apiCreateLeadsInBatches(leadsToCreate, batchSize = 10, retryAttempts = 2) {
    ...
    for (let attempt = 0; attempt <= attempts; attempt++) {
        ...
        for (let i = 0; i < pending.length; i += size) {
            ...
            // Dentro do bloco, envia em paralelo para ficar mais rÃ¡pido.
...
function normalizeImportedLeads(payload) {
    ...
    if (!inputLeads)
        return { ok: false, error: 'JSON invÃ¡lido: esperado um array de leads ou um objeto { leads: [...] }.' };
    const ids = new Set();
    const normalized = [];
...
async function importDataXlsx(file) {
    ...
    prompt += `\n\nModo: APENAS ADICIONAR (nÃ£o atualiza nem apaga os atuais).`;
    prompt += `\nLeads atuais: ${Array.isArray(existing) ? existing.length : 0} | Total apÃ³s: ${nextLeads.length}`;
...
function initDataModal() {
    ...
    if (usersCreateBtn) {
        usersCreateBtn.onclick = async () => {
            ...
            if (!isAdmin())
                return alert('Apenas admin pode criar usuÃ¡rios.');
            ...
            if (!u || !p)
                return alert('Preencha usuÃ¡rio e senha.');
            if (p.length < 6)
                return alert('Senha muito curta (mÃ­nimo 6).');
            try {
                ...
                if (e && e.code === 'user_exists')
                    return alert('Esse usuÃ¡rio jÃ¡ existe.');
                if (handleApiFailure(e, 'NÃ£o foi possÃ­vel criar o usuÃ¡rio.'))
                    return;
            }
        };
    }
    ...
    return `
        <section class="dash-hero">
            <div class="dash-hero-copy">
                ...
                <h2>VisÃ£o geral da operaÃ§Ã£o</h2>
        ...
        <div class="dash-grid dash-kpis">
            <div class="stat-card">
                ...
                <span class="meta">${totalLeads} lead(s) no perÃ­odo</span>
            ...
            <div class="stat-card warning">
                <span class="label">ConversÃ£o / Ticket MÃ©dio</span>
        ...
        <div class="dash-layout">
            <div class="chart-section dash-chart-panel">
                <div class="chart-controls">
                    <div>
                        <h4>DistribuiÃ§Ã£o Financeira por Etapa</h4>
                        <div class="hint">Troque o tipo do grÃ¡fico para comparar o peso financeiro do pipeline.</div>
                    </div>
                    ...
                    ${wonLeads.length === 0 ? '<p class="dash-empty">Nenhuma venda fechada ainda.</p>' : wonLeads.slice(-5).reverse().map(l => `
                        <div class="table-item">
                            <div class="table-main">
                                ...
                                <small>${l.origin || 'Origem nÃ£o informada'}</small>
                            </div>
                    ...
                    ${lossSummary.length === 0 ? '<p class="dash-empty">Sem registros de perda.</p>' : lossSummary.map(([reason, count]) => `
                        <div class="table-item">
                            <div class="table-main">
                                ...
                                <small>${count} ocorrÃªncia(s)</small>
                            </div>
...
async function saveUserEdit(user) {
    if (!backendOnline)
        return alert('Servidor offline.');
    if (!isAdmin())
        return alert('Apenas admin pode editar usuÃ¡rios.');
    ...
    try {
        ...
        editingUserId = '';
        alert(`UsuÃ¡rio atualizado: ${who}`);
        await refreshUsersList();
    }
    catch (e) {
        if (e && e.code === 'last_admin')
            return alert('NÃ£o Ã© possÃ­vel remover o Ãºltimo admin da funÃ§Ã£o.');
        if (e && e.code === 'weak_password')
            return alert('A nova senha precisa ter pelo menos 4 caracteres.');
        if (e && e.code === 'user_not_found')
            return alert('UsuÃ¡rio nÃ£o encontrado.');
        if (handleApiFailure(e, 'NÃ£o foi possÃ­vel atualizar o usuÃ¡rio.'))
            return;
    }
}
async function refreshUsersList() {
    ...
    try {
        ...
        if (handleApiFailure(e, 'NÃ£o foi possÃ­vel carregar usuÃ¡rios.'))
            return;
    }
}
...
async function addNewLead() {
    ...
    if (backendOnline) {
        try {
            ...
            if (handleApiFailure(e, 'NÃ£o foi possÃ­vel criar o lead no servidor.'))
                return;
            leads.push(lead);
        }
    }
...
async function addTask(id) {
    ...
    if (backendOnline) {
        try {
            ...
            if (handleApiFailure(e, 'NÃ£o foi possÃ­vel salvar tarefas no servidor.'))
                return;
        }
    }
    save();
