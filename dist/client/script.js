...
async function importDataXlsx(file) {
    ...
    const preview = buildAppendOnlyImport(existing, res.leads);
    const nextLeads = preview.merged;
    const toImport = preview.incoming;
    let prompt = `Importar ${toImport.length} lead(s) do Excel?`;
    ...
    if (backendOnline) {
        try {
            const imported = await apiCreateLeadsInBatches(toImport, 25);
...
async function importDataJson(file) {
    ...
    const nextLeads = preview.merged;
    const toImport = preview.incoming;
    let prompt = `Importar ${toImport.length} lead(s)?`;
    prompt += `\n\nModo: APENAS ADICIONAR (não atualiza nem apaga os atuais).`;
    prompt += `\nLeads atuais: ${Array.isArray(existing) ? existing.length : 0} | Total após: ${nextLeads.length}`;
    const ok = confirm(prompt);
    if (!ok)
...
function renderDashboard(container) {
    ...
    const lossSummary = Array.from(lostLeads.reduce((map, lead) => {
        const key = String(lead.lossReason || 'Sem motivo').trim();
    ...
    container.innerHTML = buildDashboardMarkupV2({
        ownerOptions,
        totalLeads,
        totalValue,
        totalWon,
    ...
    const fastSel = document.getElementById('chart-type');
    if (fastSel)
        fastSel.value = currentChartType;
    initSalesChartV2(currentChartType, viewLeads);
    return;
    container.innerHTML = `
        ...
        container.innerHTML = `
            <section class="users-view">
                ...
                <div class="data-card">
                    ...
                    <p class="users-empty">Somente administradores podem criar e remover usuários.</p>
                </div>
            </section>
        `;
        return;
    ...
    container.innerHTML = `
        <section class="users-view">
            <div class="users-header">
                <div>
                    <h2>Usuários</h2>
                    <p>Crie, revise e remova acessos sem precisar abrir o modal de dados.</p>
                </div>
            </div>
            <div class="users-grid">
                <div class="data-card" id="users-card">
                    <h4>Novo usuário</h4>
                ...
                <div class="data-card">
                    ...
                    <p>Remova logins antigos ou revise os perfis ativos no CRM.</p>
...
function setChartType(type) {
    ...
    initSalesChartV2(currentChartType, getDashboardLeads());
}
...
function initSalesChart(type = 'bar', inputLeads = leads) {
    ...
    const compactMoney = (value) => {
        ...
        if (amount >= 1000000)
            return `R$ ${(amount / 1000000).toFixed(1)} mi`;
        if (amount >= 1000)
            return `R$ ${(amount / 1000).toFixed(0)} mil`;
...
function setUsersAdminUiState() {
    ...
    const usersRole = document.getElementById('usr-role');
    if (usersCreateBtn)
        usersCreateBtn.disabled = !enabled;
    if (usersRefreshBtn)
        usersRefreshBtn.disabled = !enabled;
    if (usersName)
        usersName.disabled = !enabled;
    if (usersPass)
        usersPass.disabled = !enabled;
    if (usersRole)
        usersRole.disabled = !enabled;
}
async function createUserFromUsersView() {
    ...
    try {
        ...
        if (e && e.code === 'weak_password') {
            return alert('A senha precisa ter pelo menos 4 caracteres.');
        }
        if (handleApiFailure(e, 'Não foi possível criar o usuário.'))
            return;
    }
        ...
        btn.onclick = async () => {
            ...
            try {
                ...
                if (e && e.code === 'last_admin')
                    return alert('Não é possível remover o último admin.');
                if (handleApiFailure(e, 'Não foi possível remover o usuário.'))
                    return;
            }
        };
        ...
        options: {
            ...
            plugins: {
                ...
                tooltip: {
                    ...
                    callbacks: {
                        label: (chartCtx) => `${chartCtx.label}: ${compactMoney(chartCtx.raw || 0)}`
                    }
                }
            },
            scales: isCircular ? {} : {
            ...
            <div class="edit-body">
                <div class="edit-main">
                    <div class="field" style="margin-bottom: 12px;">
                        ...
                        <div style="font-weight: 600;">${ownerLabel}</div>
                        ${canClaimLead ? `<button class="btn-confirm btn-small" style="margin-top: 8px;" onclick='claimLead(${leadId})'>Assumir lead</button>` : ''}
                    </div>
                    <div class="field" style="margin-bottom: 12px;">
...
async function claimLead(id) {
    if (!canWrite())
        return;
    if (!currentSession || currentSession.role !== 'consultor')
        return;
    const me = sanitizeString(currentSession.user || '', 60).toLowerCase();
    if (!me)
        return;
    ...
    if (backendOnline) {
        try {
            ...
            if (handleApiFailure(e, 'Não foi possível assumir o lead no servidor.'))
                return;
            const idx = findLeadIndexById(id);
            if (idx >= 0)
                leads[idx] = next;
...
async function saveEdit(id) {
    ...
    const now = Date.now();
    if (!coerceEpochMs(lead.createdAt))
        lead.createdAt = coerceEpochMs(lead.id) ?? now;
    lead.updatedAt = now;
    if (isAdmin()) {
        const ownerEl = document.getElementById('ed-owner');
        if (ownerEl)
            lead.owner = sanitizeString(ownerEl.value, 60).toLowerCase();
    }
    if (shouldAutoClaim) {
        const me = sanitizeString(currentSession.user || '', 60).toLowerCase();
        if (me)
    ...
    if (backendOnline) {
        try {
            ...
            if (handleApiFailure(e, 'Não foi possível salvar no servidor. Salvando só localmente.'))
                return;
        }
    }
    save();
...
async function deleteLead(id) {
    ...
    if (backendOnline) {
        try {
            ...
            if (e && e.code && e.code !== 'not_found') {
                handleApiFailure(e, 'Não foi possível apagar no servidor.');
                return;
            }
        }
    }
    leads = leads.filter((l) => !leadIdEquals(l && l.id, id));
    save();
    toggleModal('modal-edit', false);
    renderApp();
}
