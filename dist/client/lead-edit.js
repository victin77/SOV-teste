function buildLossReasonOptions(lead) {
    const current = String(lead.lossReason || '').trim();
    const options = ['Preco', 'Concorrencia', 'Desistencia'];
                ...
                <div class="edit-main">
                    <div class="edit-hero">
                        <div class="edit-hero-copy">
                            <span class="edit-kicker">Lead em edicao</span>
                            <h4>${lead.name || 'Lead sem nome'}</h4>
                            <p>Atualize etapa, dados comerciais e observacoes sem perder o contexto do atendimento.</p>
                        </div>
                    ...
                    <div class="edit-summary-grid">
                        <div class="edit-summary-card edit-owner-slot">
                            <span class="edit-summary-label">Responsavel</span>
                    ...
                    <div class="edit-section-card">
                        <div class="edit-section-head">
                            <div>
                                ...
                                <p>Defina etapa, valor, telefone e proximo passo.</p>
                            </div>
                        </div>
                    ...
                    <div class="edit-section-card">
                        <div class="edit-section-head">
                            <div>
                                ...
                                <p>Use tags e observacoes para dar mais contexto ao atendimento.</p>
                            </div>
                        </div>
                        <div class="edit-form-grid">
                            <div class="edit-field edit-field-full">
                                <label>Tags</label>
                                <input type="text" id="ed-tags" value="${Array.isArray(lead.tags) ? lead.tags.join(', ') : ''}" placeholder="Ex: quente, whatsapp, indicacao">
                            </div>
                            <div class="edit-field edit-field-full">
                    ...
                    <div class="edit-actions">
                        <button class="btn-confirm" type="button" onclick='saveEdit(${leadId})'>Salvar alteracoes</button>
                ...
                <div class="edit-tasks">
                    <div class="edit-side-head">
                        <div>
                            ...
                            <p>Centralize pendencias e acompanhe o que ja foi executado.</p>
                        </div>
                    </div>
                    <div class="task-add">
...
async function claimLeadAction(id) {
    if (!canWrite())
        return;
    ...
    if (!isSharedSiteLead(lead)) {
        alert('Este lead ja foi assumido por outro consultor.');
        return;
    }
    const now = Date.now();
    ...
    if (backendOnline) {
        try {
            ...
            if (handleApiFailure(err, 'Nao foi possivel assumir o lead no servidor.'))
                return;
            const idx = findLeadIndexById(id);
            if (idx >= 0)
                leads[idx] = next;
...
async function saveEditAction(id) {
    ...
    const valueEl = document.getElementById('ed-value');
    const phoneEl = document.getElementById('ed-phone');
    const stepEl = document.getElementById('ed-step');
    const tagsEl = document.getElementById('ed-tags');
    const obsEl = document.getElementById('ed-obs');
    const lossEl = document.getElementById('ed-loss');
    ...
    if (backendOnline) {
        try {
            ...
            if (handleApiFailure(e, 'Nao foi possivel salvar no servidor. Salvando so localmente.'))
                return;
        }
    }
    save();
    ...
    if (backendOnline) {
        try {
            ...
            if (err && err.message === 'unauthorized') {
                handleApiFailure(err, 'Nao foi possivel apagar no servidor.');
                return;
            }
