            ...
            <section class="users-view">
                <div class="users-hero users-hero-locked">
                    <div>
                        ...
                        <h2>Usuarios</h2>
                        <p>Gerencie acessos do CRM em uma area dedicada.</p>
                    </div>
                </div>
                <div class="data-card users-lock-card">
                    <h4>Acesso restrito</h4>
                    <p class="users-empty">Somente administradores podem criar, editar e remover usuarios.</p>
                </div>
            </section>
        `;
        return;
        ...
        <section class="users-view">
            ...
            <div class="users-grid">
                <div class="data-card users-create-card" id="users-card">
                    ...
                    <label>Senha</label>
                    <input type="password" id="usr-pass" placeholder="Minimo 4 caracteres">
                    <label>Perfil</label>
                    <select id="usr-role">
                    ...
                    <div class="data-actions">
                        <button class="btn-confirm" id="btn-create-user" type="button" onclick="createUserFromUsersView()">Criar usuario</button>
                ...
                <div class="data-card users-list-card">
                    <div class="users-list-head">
                        <div>
                            ...
                            <p>Revise perfis ativos, remova logins antigos e ajuste permissoes.</p>
                        </div>
                    </div>
...
function updateUsersSummaryData(users) {
    ...
    if (totalEl)
        totalEl.textContent = String(safeUsers.length);
    if (adminsEl)
        adminsEl.textContent = String(safeUsers.filter((u) => u.role === 'admin').length);
    if (consultoresEl)
        consultoresEl.textContent = String(safeUsers.filter((u) => u.role === 'consultor').length);
    if (readonlyEl)
        readonlyEl.textContent = String(safeUsers.filter((u) => u.role === 'leitura').length);
}
function renderUsersListView(users) {
    ...
    if (!Array.isArray(users) || users.length === 0) {
        const meta = document.createElement('div');
        meta.className = 'users-empty-state';
        meta.textContent = 'Nenhum usuario cadastrado.';
        list.appendChild(meta);
        return;
    }
    users.forEach((u) => {
        ...
        meta.textContent = u.role === 'admin'
            ...
            : u.role === 'leitura'
                ...
                : 'Atendimento e operacao comercial';
        ...
        btn.onclick = async () => {
            ...
            if (!who)
                return;
            const ok = confirm(`Remover o usuario "${who}"?\n\nEssa acao desativa o login desse usuario.`);
            if (!ok)
                return;
            try {
                ...
                const err = e;
                if (err && err.code === 'last_admin')
                    return alert('Nao e possivel remover o ultimo admin.');
                if (handleApiFailure(err, 'Nao foi possivel remover o usuario.'))
                    return;
            }
        };
        ...
        if (editingUserId === u.user) {
            ...
            editWrap.innerHTML = `
                ...
                <div class="data-actions">
                    <button class="btn-confirm" type="button" onclick="saveUserEdit(${JSON.stringify(String(u.user || ''))})">Salvar alteracoes</button>
