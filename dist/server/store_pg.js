...
function shouldUseSsl() {
    if (process.env.PGSSL === '0' || process.env.PGSSL === 'false')
        return false;
    if (process.env.PGSSL === '1' || process.env.PGSSL === 'true')
        return true;
...
function createPool() {
    ...
    return new pg_1.Pool({ connectionString, ssl, max: 10 });
}
...
async function createUser(client, { user, role, pass }) {
    const passHash = bcryptjs_1.default.hashSync(String(pass), 10);
    await client.query(`INSERT INTO users (username, role, pass_hash, created_at)
     VALUES ($1, $2, $3, $4)
     ON CONFLICT (username) DO NOTHING`, [user, role, passHash, Date.now()]);
}
...
function createPgStore() {
    ...
    return {
        ...
        async getUser(username) {
            await init();
            const { rows } = await pool.query(`SELECT username AS user, role, pass_hash AS "passHash"
         FROM users
        ...
        async listUsers() {
            await init();
            const { rows } = await pool.query(`SELECT username AS user, role, created_at AS "createdAt"
         FROM users
        ...
        async createUser({ user, pass, role }) {
            ...
            const { rowCount } = await pool.query(`INSERT INTO users (username, role, pass_hash, created_at)
         VALUES ($1, $2, $3, $4)
         ON CONFLICT (username) DO NOTHING`, [user, role, passHash, createdAt]);
            if (!rowCount)
                return null;
        ...
        async updateUser(user, updates) {
            ...
            try {
                await client.query('BEGIN');
                const { rows } = await client.query(`SELECT username AS user, role, pass_hash AS "passHash", created_at AS "createdAt"
           FROM users
           WHERE username = $1`, [user]);
                ...
                if (current.role === 'admin' && nextRole !== 'admin') {
                    const { rows: adminRows } = await client.query(`SELECT COUNT(*)::int AS c FROM users WHERE role = 'admin'`);
                ...
                const passHash = nextPass ? bcryptjs_1.default.hashSync(String(nextPass), 10) : current.passHash;
                const { rows: updatedRows } = await client.query(`UPDATE users
           SET role = $2, pass_hash = $3
           WHERE username = $1
           RETURNING username AS user, role, created_at AS "createdAt"`, [user, nextRole, passHash]);
                await client.query('COMMIT');
                return { ok: true, user: updatedRows[0] };
            }
            catch (e) {
                try {
                    await client.query('ROLLBACK');
        ...
        async addAudit(entry) {
            ...
            const at = Date.now();
            await pool.query(`INSERT INTO audit (id, at, actor, action, entity_type, entity_id)
         VALUES ($1, $2, $3, $4, $5, $6)`, [id, at, entry.actor || '', entry.action || '', entry.entityType || '', entry.entityId || null]);
        ...
        async insertLead(storedLead, actor) {
            await init();
            const { rows } = await pool.query(`INSERT INTO leads (
        ...
        async updateLead(id, storedLead, actor) {
            await init();
            const { rows } = await pool.query(`UPDATE leads SET
          name = $2,
          phone = $3,
          origin = $4,
          value = $5,
        ...
        async softDeleteLead(id, actor) {
            ...
            const { rowCount } = await pool.query(`UPDATE leads
         SET deleted = TRUE, updated_at = $2, last_modified_by = $3
         WHERE id = $1`, [id, updatedAt, actor]);
            if (rowCount)
                await this.addAudit({ actor, action: 'lead_delete', entityType: 'lead', entityId: id });
            return rowCount > 0;
        },
        ...
        async listAudit(limit) {
            ...
            const { rows } = await pool.query(`SELECT id, at, actor, action, entity_type, entity_id
         FROM audit
         ORDER BY at DESC
         LIMIT $1`, [lim]);
